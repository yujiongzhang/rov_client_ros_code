    ...
    ,xtrans(0),ytrans(0),ztrans(0.0)
{
    ...
    vertices = loadAscllStl(":/module/rovmini.stl",1);
}
...
void AXBOpenGLWidget::initializeGL()
{
    ...
    if(!shaderprogram.addShaderFromSourceFile(QOpenGLShader::Vertex,":/module/stl.vert")){
        qDebug()<<"ERROR:"<<shaderprogram.log();    //如果编译出错,打印报错信息
    }
    if(!shaderprogram.addShaderFromSourceFile(QOpenGLShader::Fragment,":/module/stl.frag")){
        qDebug()<<"ERROR:"<<shaderprogram.log();    //如果编译出错,打印报错信息
    }
...
void AXBOpenGLWidget::paintGL()
{
    ...
    this->glClearColor(0.9373f, 0.9373f, 0.9373f, 1.0f);//设置清屏颜色
